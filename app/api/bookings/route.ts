...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ group: g, bookings: global.rows, collections: collections.rows, assignments, blacklist: blacklist.rows.map((r) => r.booking_id) })
}
...
export async function PUT(request: Request) {
  ...
  if (body.action === 'autoGroup') { const rows = await db.execute(sql`SELECT booking_id AS id, traveler, phone, email, pax, pickup FROM bookings WHERE travel_date = ${date}::date ORDER BY created_at ASC`); const existing = await db.execute(sql`SELECT cb.booking_id FROM collection_bookings cb JOIN booking_collections c ON c.id = cb.collection_id WHERE c.group_id = ${g.id} AND c.name LIKE 'Group %'`); const assignedIds = new Set(existing.rows.map((row) => String(row.booking_id))); const parse = (value: string) => { const decoded = decodeURIComponent(value || ''); const match = decoded.match(/(?:\/maps\/place\/|@|!3d)(-?\d+(?:\.\d+)?)[,!?][^\d-]*(-?\d+(?:\.\d+)?)/); return match ? { lat: Number(match[1]), lng: Number(match[2]) } : null }; const ungrouped = rows.rows.filter((row) => !assignedIds.has(String(row.id))).map((row) => ({ ...row, pax: Number(row.pax), lat: parse(String(row.pickup || ''))?.lat || 0, lng: parse(String(row.pickup || ''))?.lng || 0, leadTraveler: String(row.traveler || ''), bookingRef: String(row.id), pickupTime: null })); const buses = Array.from({ length: Math.ceil(ungrouped.reduce((sum, row) => sum + row.pax, 0) / 16) || 1 }, (_, index) => ({ id: `auto-${index + 1}`, name: `Auto group ${index + 1}`, capacity: 17 })); const result = generateGroups(ungrouped.filter((row) => row.lat !== 0 || row.lng !== 0), buses); const assignments = Object.fromEntries(result.groups.flatMap((group, index) => group.bookings.map((booking) => [booking.bookingRef || booking.id, index + 1]))); const merged = { ...Object.fromEntries(existing.rows.map((row) => [String(row.booking_id), 1])), ...assignments }; await db.execute(sql`DELETE FROM booking_collections WHERE group_id = ${g.id} AND name LIKE 'Group %'`); for (let i = 1; i <= buses.length; i++) { const created = await db.execute(sql`INSERT INTO booking_collections (group_id, name) VALUES (${g.id}, ${`Group ${i}`}) RETURNING id`); for (const [bookingId, groupNumber] of Object.entries(merged)) if (Number(groupNumber) === i) await db.execute(sql`INSERT INTO collection_bookings (collection_id, booking_id, traveler, phone, email, pax, pickup) SELECT ${created.rows[0].id}, booking_id, traveler, phone, email, pax, pickup FROM bookings WHERE booking_id = ${bookingId} AND travel_date = ${date}::date ON CONFLICT DO NOTHING`) } return NextResponse.json({ groupsCreated: result.groups.length, mixedPickupTimes: result.groups.filter((group) => group.mixedPickupTimes).length, englishGroups: result.groups.filter((group) => group.englishFocused).length, skipped: rows.rows.length - ungrouped.filter((row) => row.lat !== 0 || row.lng !== 0).length }) }
  if (body.action === 'saveAssignments') { const assignments = body.assignments || {}; await db.execute(sql`DELETE FROM booking_collections WHERE group_id = ${g.id} AND name LIKE 'Group %'`); for (let i = 1; i <= 20; i++) { const c = await db.execute(sql`INSERT INTO booking_collections (group_id, name) VALUES (${g.id}, ${`Group ${i}`}) RETURNING id`); const collectionId = (c.rows[0] as { id: number }).id; for (const [bookingId, groupNumber] of Object.entries(assignments)) { if (Number(groupNumber) === i) await db.execute(sql`INSERT INTO collection_bookings (collection_id, booking_id, traveler, phone, email, pax, pickup) SELECT ${collectionId}, booking_id, traveler, phone, email, pax, pickup FROM bookings WHERE booking_id = ${bookingId} AND travel_date = ${date}::date ON CONFLICT DO NOTHING`) } } return NextResponse.json({ ok: true }) }
